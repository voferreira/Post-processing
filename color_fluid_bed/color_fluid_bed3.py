...
def modify(case_path):
    ...
    if "alumina" in case_path:
        #first = 60
        step = 2
        reference_time_step = 25
        criterion = 0.95
        last = None
        folder_data = i.split('/')[9].split('_')[3]
        
    elif "A-q1-q1-lift-old" in case_path:
        #first = 30
        reference_time_step = 20
        step = 2
        criterion = 0.5
        last = None
        folder_data = i.split('/')[8].split('_')[3]
    elif "/e" in case_path:
        #first = 50
        reference_time_step = 25
        step = 2
    ...
    particles = lethe_pyvista_tools(case_path = case_path, prm_file_name = "liquid_fluidized_bed.prm", pvd_name = "result__particles.pvd", step = step, last = last, n_procs = 5)
    ...
    df_height = pd.DataFrame({"simulation" : folder_data, "n_flows" : n_flows_height, "time" : time_height}, index=[0])
    df_radius = pd.DataFrame({"simulation" : folder_data, "n_flows" : n_flows_radius, "time": time_radius}, index=[0])
    df_theta = pd.DataFrame({"simulation" : folder_data, "n_flows" : n_flows_theta, "time": time_theta}, index=[0])
    df_doucet = pd.DataFrame({"simulation" : folder_data, "n_flows" : n_flows_doucet, "time": time_doucet}, index=[0])

    if "alumina" in i.split("/") or "Alumina" in i.split("/"):
        folder = "/mnt/e/backup/2022-10-14/results_beluga_to_organize/scratch/alumina/A-q1-q1-lift/n_flows.xlsx"

    elif "A-q1-q1-lift-old" in i.split("/"):
        folder = "/mnt/e/backup/2022-10-14/results_beluga_to_organize/scratch/A-q1-q1-lift-old/n_flows.xlsx"
    ...
    plt.legend()
    plt.savefig(f"{case_path}/{folder_data}.png")
    plt.close()
    ...
    plt.ylabel("Number of particles")
    plt.legend()
    plt.savefig(f"{case_path}/{folder_data}_flow.png")
    plt.close()
    ...
    plt.legend()
    plt.savefig(f"{case_path}/{folder_data}_mixing_index.png")
    plt.close()
