...
def modify(case_path):
    ...
    if "alginate" in case_path:
        #first = 60
        reference_time_step = 60
    elif "/e" in case_path:
        #first = 50
        reference_time_step = 50
        print("NEW \n")
    else:
        #first = 30
        reference_time_step = 30

    print("\n")
    print(f"WORKING ON: {i}")
    print("\n")

    particles = lethe_pyvista_tools(case_path = case_path, prm_file_name = "liquid_fluidized_bed.prm", pvd_name = "result__particles.pvd")

    #particles.read_lethe_to_pyvista(pvd_name = "result__particles.pvd", first = first)

    reference_time_step = 30#30

    df = particles.get_df(reference_time_step)

    split_height = np.mean(df.points[:, 0])
...
'''
    ...
    pbar = tqdm(total = len(particles.df), desc = "Counting results")
    for i_df, df in enumerate(particles.df):
        ...
        if i_df > reference_time_step:
            if n_flows_height < 0 and height_mixing_index[i_df] >= 0.95:
                n_flows_height = flow_throughs[i_df]
                time_height = particles.time_list[i_df] - particles.time_list[reference_time_step]

            if n_flows_radius < 0 and radius_mixing_index[i_df] >= 0.95:
                n_flows_radius = flow_throughs[i_df]
                time_radius = particles.time_list[i_df] - particles.time_list[reference_time_step]

            if n_flows_theta < 0 and theta_mixing_index[i_df] >= 0.95:
                n_flows_theta = flow_throughs[i_df]
                time_theta = particles.time_list[i_df] - particles.time_list[reference_time_step]
            
            if n_flows_doucet < 0 and doucet_mixing_index[i_df] >= 0.95:
                n_flows_doucet = flow_throughs[i_df]
                time_doucet = particles.time_list[i_df] - particles.time_list[reference_time_step]

        pbar.update(1)
    
    df_height = pd.DataFrame({"simulation" : i.split('/')[5] + '_' + i.split('/')[4] + '_' + i.split('/')[6].split('_')[-1], "n_flows" : n_flows_height, "time" : time_height}, index=[0])
    df_radius = pd.DataFrame({"simulation" : i.split('/')[5] + '_' + i.split('/')[4] + '_' + i.split('/')[6].split('_')[-1], "n_flows" : n_flows_radius, "time": time_radius}, index=[0])
    df_theta = pd.DataFrame({"simulation" : i.split('/')[5] + '_' + i.split('/')[4] + '_' + i.split('/')[6].split('_')[-1], "n_flows" : n_flows_theta, "time": time_theta}, index=[0])
    df_doucet = pd.DataFrame({"simulation" : i.split('/')[5] + '_' + i.split('/')[4] + '_' + i.split('/')[6].split('_')[-1], "n_flows" : n_flows_doucet, "time": time_doucet}, index=[0])

    if "alginate" in i.split("/") or "Alginate" in i.split("/"):
        folder = "/mnt/d/particles_props/alginate/n_flows.xlsx"
    else:
        folder = "/mnt/d/particles_props/n_flows.xlsx"
    ...
    np.savetxt(f"{i}/data.csv", np.transpose(data), delimiter=',', header = "time, n_ID_height, n_ID_radius, n_ID_theta, height_mixing_index, radius_mixing_index, theta_mixing_index,doucet_mixing_index, doucet_eigenvector_r, doucet_eigenvector_theta, doucet_eigenvector_z, height_mixing_index_std, radius_mixing_index_std, theta_mixing_index_std", comments='', fmt='%1.6f')#  doucet_mixing_index, doucet_eigenvector_r, doucet_eigenvector_theta, doucet_eigenvector_z,
    ...
    plt.plot(particles.time_list[reference_time_step:], len(particles.df[reference_time_step]["ID"])/4 * doucet_mixing_index[reference_time_step:], label = "Doucet")
    plt.plot(particles.time_list[reference_time_step:], np.repeat(len(particles.df[reference_time_step]["ID"])/4, len(particles.time_list[reference_time_step:])), ':', label = "A fourth of particles amount")
    plt.xlabel(r"Time ($s$)")
    ...
    plt.plot(flow_throughs[reference_time_step:], len(particles.df[reference_time_step]["ID"])/4 * doucet_mixing_index[reference_time_step:], label = "Doucet")
    plt.plot(flow_throughs[reference_time_step:], np.repeat(len(particles.df[reference_time_step]["ID"])/4, len(particles.time_list[reference_time_step:])), ':', label = "A fourth of particles amount")
    ...
    plt.plot(flow_throughs[reference_time_step:], np.repeat(1, len(particles.time_list[reference_time_step:])), ':', label = "A fourth of particles amount")
    plt.xlabel(r"Number of flows through")
